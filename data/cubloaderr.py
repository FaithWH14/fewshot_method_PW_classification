...
########################################
parser = get_arg()
SUPPORT_NUM = parser.support_num
num_query = parser.val_support_num
########################################
def pil_loader(path):
...
class ImageLoader(torch.utils.data.Dataset):
    def __init__(self, root, transform=None, att_transform=None,  base_one=False, train=False, num_classes=2, num_train_sample=0, 
        ...
        base_data = data[data['label'] < 2]
        novel_data = data[data['label'] <= 2]
        ...
        if (num_train_sample != 0):
            ...
            base_one_data = base_data.groupby('label', group_keys=False).apply(lambda x: x.iloc[:num_train_sample])
